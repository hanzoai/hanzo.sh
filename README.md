...
| `dev` | — | [hanzoai/dev](https://github.com/hanzoai/dev) |
...
curl -fsSL https://hanzo.sh/dev | sh
```
...
The page is Vite + React 19 on **@hanzo/ui** over the **@hanzo/gui** backend,
with **@hanzo/design** tokens (monochrome, dark-default, self-hosted Geist), the
**@hanzo/logo** mark via `BrandMark`, and the header/footer content from
**@hanzo/products** so this property cannot drift from the rest of the estate. No
Tailwind, no shadcn, no Radix. One route. `src/site.ts` is the page as data —
the install commands, the tools, what is not installed yet — and everything in
`src/components/` only renders it.

```sh
pnpm install
pnpm dev        # vite, :8080
pnpm build      # -> dist/, including the polyglot rewrite
pnpm typecheck  # tsgo, the native TypeScript compiler
pnpm lint
```
